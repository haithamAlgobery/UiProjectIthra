...
// import { fetchCommentsByContent, postComment, clearComments, editComment, deleteComment } from "@/src/features/commentsSlice";
...
//   const { content, loading, error } = useSelector((s) => s.viewContent || { content: null, loading: false, error: null });
...
// if(fetchContentFull)
//   dispatch(sendViewsSingle(contentId))
...
//           <Card className="overflow-hidden shadow-xl border-0 bg-card mb-8 rounded-2xl">
//             {/* Hero Image / Video */}
//             {heroUrl && (
//               <div className="relative w-full aspect-[21/9] bg-muted">
//                 {heroIsVideo ? (
//                   <video src={heroUrl} controls className="w-full h-full object-cover" />
//                 ) : (
//                   <img src={heroUrl} alt={c.title} className="w-full h-full object-cover" />
//                 )}
...
//                     <Badge className="bg-white/90 text-foreground backdrop-blur-sm shadow-lg px-4 py-1.5 text-sm font-medium rounded-full">
//                       {content.category.title}
//                     </Badge>
//                   </div>
//                 )}
...
//                     <div className="font-bold text-lg text-foreground">{userShort.firstName} {userShort.lastName}</div>
...
//                     className="p-2.5 rounded-full hover:bg-muted transition-all duration-200 hover:scale-105"
//                     aria-label="المزيد"
//                   >
...
//                       <div className="absolute left-0 mt-2 w-56 bg-card border border-border rounded-xl shadow-2xl py-2 z-20 animate-in fade-in slide-in-from-top-2 duration-200">
//                         <button
//                           onClick={() => {
//                             handleCopyLink();
//                             setShowMenu(false);
...
//               <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold leading-tight text-balance text-foreground mb-4">{c.title}</h1>
...
//                 </div>
//                 <div className="flex items-center gap-3">
//                   {c.dateUpdate && c.dateUpdate !== c.dateCreate && <span className="text-xs"> اخر تعديل {getRelativeTime(c.dateUpdate)}</span>}
...
//               {/* Tags: عرض قائمة الوسوم تحت المحتوى وفوق أزرار التفاعل */}
//               {Array.isArray(content.tags) && content.tags.length > 0 && (
//                 <div className="mt-6 mb-4 flex flex-wrap gap-2">
//                   {content.tags.map((t) => (
//                     <Badge key={t} className="bg-muted/60 text-foreground/90 rounded-full px-3 py-1.5 text-sm">
//                       #{t}
//                     </Badge>
//                   ))}
//                 </div>
...
//                               <div className="flex-1 min-w-0">
//                                 <div className="font-semibold text-foreground group-hover:text-primary transition-colors mb-1">{r.title}</div>
//                                 {r.details && <div className="text-sm text-muted-foreground line-clamp-2">{r.details}</div>}
//                               </div>
...
//                       myInterActive.isLike ? "bg-green-500/10 text-green-600 dark:text-green-400" : "hover:bg-muted text-muted-foreground hover:text-foreground"
//                     )}
//                     aria-pressed={myInterActive.isLike}
//                   > 
//                     <ThumbsUp className={cn("h-5 w-5 transition-all duration-200", myInterActive.isLike && "fill-current")} />
...
//                     className={cn(
//                       "flex items-center gap-2 px-4 py-2.5 rounded-full transition-all duration-200 hover:scale-105",
//                       myInterActive.isNotLike ? "bg-red-500/10 text-red-600 dark:text-red-400" : "hover:bg-muted text-muted-foreground hover:text-foreground"
//                     )}
//                     aria-pressed={myInterActive.isNotLike}
//                   >
//                     <ThumbsDown className={cn("h-5 w-5 transition-all duration-200", myInterActive.isNotLike && "fill-current")} />
...
//                       myInterActive.isLove ? "bg-amber-500/10 text-amber-600 dark:text-amber-400" : "hover:bg-muted text-muted-foreground hover:text-foreground"
//                     )}
//                     aria-label="حفظ"
//                   >
...
//                         disabled={!isAuth}
//                         className="w-full p-4 bg-background border-2 border-border focus:border-primary rounded-xl resize-none focus:outline-none transition-colors text-foreground placeholder:text-muted-foreground disabled:opacity-50 disabled:cursor-not-allowed"
//                         rows={4}
//                       />
//                     </div>
//                     <div className="flex items-center justify-between mt-3">
//                       <p className="text-xs text-muted-foreground">{isAuth ? "كن محترمًا ومهذبًا في تعليقك" : "يجب تسجيل الدخول للتعليق"}</p>
//                       <div className="flex items-center gap-2">
//                         <Button size="sm" variant="ghost" onClick={() => setCommentText("")} disabled={!commentText.trim()}>
//                           إلغاء
//                         </Button>
//                         <Button size="sm" onClick={handlePostComment} disabled={commentsState.posting || !commentText.trim() || !isAuth} className="gap-2 shadow-sm">
...
//                         <div key={cm.id} className="flex gap-4 p-5 bg-muted/30 border border-border rounded-xl hover:bg-muted/50 transition-colors">
//                           <Avatar className="h-11 w-11 ring-2 ring-border flex-shrink-0">
//                             <AvatarFallback className="bg-primary/10 text-primary text-sm font-semibold">
//                               {cm.userInitial || (cm.userName?.[0] ?? cm.userId?.[0] ?? "ا").toUpperCase()}
//                             </AvatarFallback>
//                           </Avatar>
//                           <div className="flex-1 min-w-0 space-y-2">
//                             <div className="flex items-center justify-between gap-2">
//                               <div className="flex items-center gap-2 flex-1 min-w-0">
//                                 <span className="font-bold text-sm text-foreground truncate">{cm.userName || cm.userId}</span>
//                                 <span className="text-muted-foreground/40">•</span>
//                                 <time className="text-xs text-muted-foreground flex-shrink-0">{getRelativeTime(cm.dateCreated)}</time>
//                               </div>
...
//           <div className="px-4 py-2 rounded-lg shadow-lg bg-card border border-border text-sm">
//             {toastMessage}
//           </div>
//         </div>
//       )}
